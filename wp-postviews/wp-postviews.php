...
if(!function_exists('get_most_viewed')) {
	function get_most_viewed($mode = '', $limit = 10, $offset = 0,$temps=0, $chars = 0, $display = true) {
		global $wpdb;
		$views_options = get_option('views_options');
		$where = '';
		$temp = '';
		...
		$most_viewed = $wpdb->get_results("SELECT DISTINCT $wpdb->posts.*,(meta_value+0) AS views FROM $wpdb->posts LEFT JOIN $wpdb->postmeta ON $wpdb->postmeta.post_id = $wpdb->posts.ID LEFT JOIN $wpdb->term_relationships ON $wpdb->term_relationships.object_id = $wpdb->posts.ID WHERE term_taxonomy_id!=33 AND post_date < '".current_time('mysql')."' AND $where AND post_status = 'publish' AND meta_key = 'views' AND post_password = '' ORDER BY views DESC LIMIT $offset,$limit");
		if($most_viewed) {
			foreach ($most_viewed as $post) {
				...
				$temp = stripslashes($views_options['most_viewed_template']);
				if($temps == 1) {
				$temp ='<li><a href="%POST_URL%"><span class="a-img" style="background-image:url(%POST_thumbnail%);"><img src="http://h5.appgame.com/wp-content/themes/gamezone-h5/images/icon-default.png"></span><span class="a-text">%POST_TITLE%</span></a></li>';
				}
				$temp = str_replace("%VIEW_COUNT%", number_format_i18n( $post_views ), $temp);
				$temp = str_replace("%VIEW_COUNT_ROUNDED%", postviews_round_number( $post_views ), $temp);
				$temp = str_replace("%POST_TITLE%", $post_title, $temp);
				...
				$temp = str_replace("%POST_CONTENT%", $post->post_content, $temp);
				$temp = str_replace("%POST_URL%", get_permalink($post), $temp);
				$temp = str_replace("%POST_TYPE%", $type, $temp);
				$temp = str_replace("%POST_ID%", $post->ID, $temp);
				$temp = str_replace("%POST_PLAYLINK%", $substr, $temp);
$thumbnail = '';
if ( has_post_thumbnail($post->ID)) { $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'thumbnail');$thumbnail = $large_image_url[0];}
